...
namespace OpenSage
{
    public sealed class Scene3D : DisposableBase
    {
        ...
        {
            DrawUnitStatus(drawingContext);

            SelectionGui?.Draw(drawingContext);
            DebugOverlay?.Draw(drawingContext, Camera);
        }

        private void DrawUnitStatus(DrawingContext2D drawingContext)
        {
            ...
            if (LocalPlayer != null)
            {
                ...
                {
                    // todo: this is expensive - probably time to start caching
                ...
                foreach (var obj in GameObjects.Items)
                {
                    ...
                    DrawAnimations(drawingContext, obj, GameContext.GameLogic.CurrentFrame.Value);
                }
            }
        }

        private static BoundingSphere GetBoundingSphere(GameObject gameObject)
        {
