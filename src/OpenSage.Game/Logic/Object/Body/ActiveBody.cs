...
public class ActiveBody : BodyModule
{
    ...
    public override DamageInfoOutput AttemptDamage(in DamageInfoInput damageInput)
    {
            ...
            case DamageType.Status:
                {
                    ...
                    // DamageStatusType.
                    var framesToStatusFor = LogicFrameSpan.FromMilliseconds(amount);
        ...
        if (amount > 0.0f || damageInput.Kill)
        {
            ...
            if (_lastDamageInfo.Request.SourceID != 0)
            {
            ...
            {
                // 25% chance to play.
                if (GameEngine.Random.Next(100) < 25)
                {
                    GameEngine.AudioSystem.PlayAudioEvent(
                        obj.Translation,
    ...
    public override uint ClearableLastAttacker => _lastDamageCleared ? 0 : _lastDamageInfo.Request.SourceID;
