...
/// </summary>
public sealed class LookAtTranslator(RtsCameraController cameraController, CursorManager cursorManager, GameData gameData) : InputMessageHandler
{
    private readonly RtsCameraController _cameraController = cameraController;
    ...
    private void SetScrolling(ScrollType scrollType)
    {
        ...
        _cameraController.MouseLock = true;
    }
    ...
    private void StopScrolling(in TimeInterval gameTime)
    {
        ...
        _cameraController.MouseLock = false;
            ...
            case InputMessageType.MouseMiddleButtonUp:
                {
                    ...
                    {
                        _cameraController.SetAngleAndPitchToDefault();
                        _cameraController.SetZoomToDefault();
                    }
            ...
            case InputMessageType.MouseMove:
                {
                    ...
                    var screenSize = _cameraController.Camera.ScreenSize;
    ...
    public void Update(in TimeInterval gameTime)
    {
        ...
        else if (IsScrolling)
        {
                ...
                case ScrollType.RightMouseButton:
                    {
                        ...
                        {
                            var max = _cameraController.Camera.ScreenSize / 2.0f;
                            _anchor = new Point2D(
                                (int)Math.Clamp(_anchor.X, max.X, _cameraController.Camera.ScreenSize.X - max.X),
                                (int)Math.Clamp(_anchor.Y, max.Y, _cameraController.Camera.ScreenSize.Y - max.Y)
                            );
                        }
            ...
            // TODO(Port): TheInGameUI->setScrollAmount(offset);
            _cameraController.ScrollBy(offset, gameTime);
        }
        else
        {
