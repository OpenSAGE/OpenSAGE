...
namespace OpenSage.Data.Ini
{
    internal sealed partial class IniParser
    {
        ...
        public LazyAssetReference<CommandButton> ParseCommandButtonReference()
        {
            ...
            return new LazyAssetReference<CommandButton>(() => AssetStore.CommandButtons.GetByKey(name));
        }

        public LazyAssetReference<CommandSet> ParseCommandSetReference()
        {
            var name = ParseAssetReference();
            return new LazyAssetReference<CommandSet>(() => AssetStore.CommandSets.GetByKey(name));
        }

        public LazyAssetReference<Upgrade> ParseUpgradeReference()
        {
            var name = ParseAssetReference();
            return new LazyAssetReference<Upgrade>(() => AssetStore.Upgrades.GetByKey(name));
        }

        public LazyAssetReference<Locomotor> ParseLocomotorReference()
        {
            var name = ParseAssetReference();
            return new LazyAssetReference<Locomotor>(() => AssetStore.Locomotors.GetByKey(name));
        }
        ...
        public LazyAssetReference<Locomotor>[] ParseLocomotorReferenceArray()
        {
            ...
            while ((token = GetNextTokenOptional()).HasValue)
            {
                ...
                result.Add(new LazyAssetReference<Locomotor>(() => AssetStore.Locomotors.GetByKey(localToken.Value.Text)));
            }
        ...
        {
            var name = ParseAssetReference();
            return new LazyAssetReference<BaseAudioEventInfo>(() => AssetStore.AudioEvents.GetByKey(name));
        }

        public LazyAssetReference<AudioFile> ParseAudioFileReference()
        {
            var name = ParseAssetReference();
            return new LazyAssetReference<AudioFile>(() => AssetStore.AudioFiles.GetByKey(name));
        }
        ...
        public LazyAssetReference<AudioFileWithWeight>[] ParseAudioFileReferenceArray()
        {
            ...
            while ((token = GetNextTokenOptional()).HasValue)
            {
                ...
                {
                    AudioFile = new LazyAssetReference<AudioFile>(() => AssetStore.AudioFiles.GetByKey(localToken.Value.Text))
                }));
            }
        ...
        {
            var name = ParseAssetReference();
            return new LazyAssetReference<ObjectDefinition>(() => AssetStore.ObjectDefinitions.GetByKey(name));
        }

        public LazyAssetReference<Texture> ParseTextureReference()
        {
            var fileName = ParseFileName();
            return new LazyAssetReference<Texture>(() => AssetStore.Textures.GetByKey(fileName));
        }

        public LazyAssetReference<Texture> ParseGuiTextureReference()
        {
            var fileName = ParseFileName();
            return new LazyAssetReference<Texture>(() => AssetStore.GuiTextures.GetByKey(fileName));
        }
        ...
        public LazyAssetReference<Model> ParseModelReference()
        {
            ...
            return (!string.Equals(fileName, "NONE", StringComparison.OrdinalIgnoreCase))
                ? new LazyAssetReference<Model>(() => AssetStore.Models.GetByKey(fileName))
                : null;
        }
        ...
        public LazyAssetReference<Graphics.Animation.Animation> ParseAnimationReference()
        {
            ...
            return (!string.Equals(animationName, "NONE", StringComparison.OrdinalIgnoreCase))
                ? new LazyAssetReference<Graphics.Animation.Animation>(() => AssetStore.Animations.GetByKey(animationName))
                : null;
        }
