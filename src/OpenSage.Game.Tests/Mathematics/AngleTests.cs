...
namespace OpenSage.Tests.Mathematics
{
    public class AngleTests
    {
        ...
        {
            Assert.Equal(90.0f, MathUtility.ToDegrees(MathUtility.PiOver2), Precision);
        }
        ...
        {
            Assert.Equal(angle, MathUtility.ToDegrees(MathUtility.GetYawFromDirection(new Vector2(x, y))), Precision);
        }
        ...
        public void GetPitchFromDirectionTest(float x, float y, float z, float angle)
        {
            ...
            Assert.Equal(angle, MathUtility.ToDegrees(MathUtility.GetPitchFromDirection(vec)), Precision);
        }
